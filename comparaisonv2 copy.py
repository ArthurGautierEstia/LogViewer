...
# 1. CONFIGURATION
# ==============================================================================
APT_FILE_PATH = 'v10.aptsource'
RSI_JSON_PATH = 'kuka_log_cleancvel94v10.json'
ORIGIN_OFFSET = (0, 0, 0)
ORIGIN_ROTATION_DEG = (0, 0, 0)
INTERPOLATION_STEP_MM = 1
...
def resample_path(df, step_mm=1):
    """Interpoler une trajectoire (DataFrame avec colonnes X,Y,Z) à pas régulier."""
    coords = df[['X', 'Y', 'Z']].values
    dists = np.linalg.norm(np.diff(coords, axis=0), axis=1)
    cumdist = np.insert(np.cumsum(dists), 0, 0)
    total_length = cumdist[-1]
    new_distances = np.arange(0, total_length, step_mm)
    new_coords = np.empty((len(new_distances), 3))
    for i in range(3):
        new_coords[:, i] = np.interp(new_distances, cumdist, coords[:, i])
    # Interpoler aussi les autres colonnes si besoin
    new_df = pd.DataFrame(new_coords, columns=['X', 'Y', 'Z'])
    return new_df

def project_point_on_segment(p, a, b):
    """Projette le point p sur le segment [a,b] et retourne la projection et la distance."""
    ab = b - a
    ap = p - a
...
# def synchronize_by_local_projection(theo_df, real_df, window=5):
#     """Pour chaque point théorique, projette localement sur la trajectoire réelle autour de l'index correspondant."""
#     results = []
#     real_coords = real_df[['X', 'Y', 'Z']].values
#     theo_coords = theo_df[['X', 'Y', 'Z']].values
#     for i, p in enumerate(theo_coords):
#         i_min = max(0, i - window)
#         i_max = min(len(real_coords) - 2, i + window)
#         min_dist = np.inf
#         best_proj = None
#         best_j = None
#         for j in range(i_min, i_max):
#             a, b = real_coords[j], real_coords[j+1]
#             proj, dist = project_point_on_segment(p, a, b)
#             if dist < min_dist:
#                 min_dist = dist
#                 best_proj = proj
#                 best_j = j
#         # On peut aussi interpoler la vitesse réelle entre j et j+1 si besoin
#         results.append({
#             'X_th': p[0], 'Y_th': p[1], 'Z_th': p[2],
#             'X_real': best_proj[0], 'Y_real': best_proj[1], 'Z_real': best_proj[2],
#             'Positional_Error': min_dist,
#             'Real_index': best_j
#         })
#     return pd.DataFrame(results)

def synchronize_by_local_projection(theo_df, real_df, window=10):
    results = []
    real_coords = real_df[['X', 'Y', 'Z']].values
    theo_coords = theo_df[['X', 'Y', 'Z']].values
    # On récupère la vitesse réelle si elle existe
    has_speed = 'RealSpeed_mm_s' in real_df.columns
    real_speeds = real_df['RealSpeed_mm_s'].values if has_speed else None
    theo_speeds = theo_df['Theoretical_Speed_mm_s'].values if 'Theoretical_Speed_mm_s' in theo_df.columns else None
    ...
    for i, p in enumerate(theo_coords):
        ...
        for j in range(i_min, i_max):
            ...
            if dist < min_dist:
                ...
                best_proj = proj
                best_j = j
                # Interpolation linéaire de la vitesse réelle si possible
                if has_speed:
                    ab = b - a
                    ap = p - a
                    t = np.dot(ap, ab) / np.dot(ab, ab)
                    t = np.clip(t, 0, 1)
                    best_speed = real_speeds[j] * (1 - t) + real_speeds[j+1] * t
        # Fallback si aucune projection trouvée
        #print(f" BEST proj != None ; Point {i}: min_dist={min_dist:.2f}mm, best_j={best_j}")
        if best_proj is None:
            dists = np.linalg.norm(real_coords - p, axis=1)
            idx = np.argmin(dists)
            best_proj = real_coords[idx]
            min_dist = dists[idx]
            best_j = idx
            best_speed = real_speeds[idx] if has_speed else None
            #print(f" BEST proj = None ; Point {i}: min_dist={min_dist:.2f}mm, best_j={best_j}")
        # Calcul de l'erreur de vitesse
        theo_speed = theo_speeds[i] if theo_speeds is not None else None
        speed_error = (best_speed - theo_speed) if (best_speed is not None and theo_speed is not None) else None
        results.append({
            'X_th': p[0], 'Y_th': p[1], 'Z_th': p[2],
            'X_real': best_proj[0], 'Y_real': best_proj[1], 'Z_real': best_proj[2],
            'Positional_Error': min_dist,
            'Real_index': best_j,
            'RealSpeed_mm_s': best_speed,
            'Theoretical_Speed_mm_s': theo_speed,
...
if __name__ == "__main__":
    ...
    if apt_segments:
        ...
        if full_rsi_df is not None:
            ...
            resampled_theoretical_df = resample_path(dense_theoretical_df, step_mm=0.1)
            resampled_real_df = resample_path(trimmed_rsi_df, step_mm=0.1)
            comparison_df = synchronize_by_local_projection(resampled_theoretical_df, resampled_real_df, window=5)
